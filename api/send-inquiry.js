// /api/send-inquiry.js
// Vercel serverless handler to email inquiries using SendGrid's REST API (no npm deps).
// Set these ENV VARS in Vercel Project Settings:
//   - SENDGRID_API_KEY (required)
//   - INQUIRY_TO (default: info@incluu.us)
//   - INQUIRY_FROM (default: info@incluu.us)  // can be the same verified sender; no custom domain needed
//
// POST JSON body: { tier, name, email, message?, honeypot? }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (honeypot) return res.status(200).json({ ok:true }); // simple anti-bot
    ...
    const payload = {
      personalizations: [
        { to: [{ email: TO }], subject: `RAR Retreat Inquiry — ${tier}` }
      ],
