...
function scrollToReserve(e){ e?.preventDefault?.(); document.getElementById('reserve')?.scrollIntoView({behavior:'smooth'}); }

/* Simple “force-show or fallback” <img> */
...
export default function Page() {
  ...
  const PRICE = { t1: 7600, t2: 10100, t3: 12400 };
  ...
  const pick = (t)=> (e)=>{ setSelectedTier(t); scrollToReserve(e); };
    ...
    <div className="min-h-screen w-full">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10" aria-hidden>
          <SafeImg
            src="https://images.unsplash.com/photo-1470115636492-6d2b56f9146e?q=80&w=2400&auto=format&fit=crop"
            fallback="/images/hero_placeholder.svg"
            alt="Misty forest in Japan"
        ...
        <div className="relative section py-24 lg:py-36">
          <div className="max-w-3xl">
            ...
            <div className="mt-6 text-sm text-white/80 space-x-3">
              ...
              <span className="badge"><span className="inline-block h-2 w-2 rounded-full bg-[#D4A373]" /> Early‑Bird live</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="section">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="card-warm">
              ...
              <div className="mt-3 text-4xl font-bold">${PRICE.t1.toLocaleString()} <span className="text-base font-medium text-white/60">pp</span></div>
              <ul className="mt-4 space-y-2 text-sm text-white/80">
                <li>• Boutique hotels + ryokans (shared at Sakura‑Sakura)</li>
                <li>• Onsen ritual in Shichirigahama & Beppu</li>
              ...
              <a href="#reserve" onClick={pick('t1')} className="mt-6 inline-block btn-primary w-full text-center">Inquiry</a>
              <a href={podiaCheckoutUrl('podia_deposit_button','t1')} target="_blank" rel="noreferrer" className="mt-2 inline-block w-full text-center rounded-2xl border border-[#F1C376] text-[#F1C376] px-4 py-3 font-semibold hover:bg-[#F1C376]/10">Pay Deposit</a>
            </div>
            ...
            <div className="card-gold">
              ...
              <div className="mt-3 text-4xl font-bold">${PRICE.t2.toLocaleString()} <span className="text-base font-medium text-white/60">pp</span></div>
              <ul className="mt-4 space-y-2 text-sm text-white/80">
              ...
              <a href="#reserve" onClick={pick('t2')} className="mt-6 inline-block btn-primary w-full text-center">Inquiry</a>
              <a href={podiaCheckoutUrl('podia_deposit_button','t2')} target="_blank" rel="noreferrer" className="mt-2 inline-block w-full text-center rounded-2xl border border-[#F1C376] text-[#F1C376] px-4 py-3 font-semibold hover:bg-[#F1C376]/10">Pay Deposit</a>
            </div>
            ...
            <div className="card-warm">
              ...
              <div className="mt-3 text-4xl font-bold">${PRICE.t3.toLocaleString()} <span className="text-base font-medium text-white/60">pp</span></div>
              <ul className="mt-4 space-y-2 text-sm text-white/80">
              ...
              <a href="#reserve" onClick={pick('t3')} className="mt-6 inline-block btn-primary w-full text-center">Inquiry</a>
              <a href={podiaCheckoutUrl('podia_deposit_button','t3')} target="_blank" rel="noreferrer" className="mt-2 inline-block w-full text-center rounded-2xl border border-[#F1C376] text-[#F1C376] px-4 py-3 font-semibold hover:bg-[#F1C376]/10">Pay Deposit</a>
            </div>
          </div>
        </div>
      </section>

      {/* Photorealistic gallery with graceful fallbacks */}
      <section className="section py-10">
        <div className="grid md:grid-cols-3 gap-4">
          <SafeImg
            src="https://images.unsplash.com/photo-1548783305-6bfb9a7b8a02?q=80&w=1400&auto=format&fit=crop"
            fallback="/images/kamakura_fallback.svg"
            alt="Kamakura coast"
            className="rounded-2xl border border-white/10 object-cover w-full h-56"
          />
          <SafeImg
            src="https://images.unsplash.com/photo-1590559899731-2fc8b1a79cd4?q=80&w=1400&auto=format&fit=crop"
            fallback="/images/beppu_fallback.svg"
            alt="Beppu onsen steam"
            className="rounded-2xl border border-white/10 object-cover w-full h-56"
          />
          <SafeImg
            src="https://images.unsplash.com/photo-1536098561742-ca998e48cbcc?q=80&w=1400&auto=format&fit=crop"
            fallback="/images/miyajima_fallback.svg"
            alt="Miyajima island"
            className="rounded-2xl border border-white/10 object-cover w-full h-56"
          />
        </div>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            ...
            <div className="card-gold">
              ...
              <div className="mt-2 space-y-1 text-lg">
                <div>• Tier 1 — ${PRICE.t1.toLocaleString()}</div>
                <div>• Tier 2 — ${PRICE.t2.toLocaleString()}</div>
                <div>• Tier 3 — ${PRICE.t3.toLocaleString()}</div>
              </div>
      ...
      {/* FAQ trimmed for brevity — unchanged */}
            ...
            <form onSubmit={submitForm} className="space-y-4 mt-6">
              <div>
                ...
                <select value={selectedTier} onChange={(e)=>setSelectedTier(e.target.value)} className="mt-1 w-full rounded-xl bg-black/30 border border-white/20 px-4 py-3">
                  <option value="t1">Tier 1 – Essential — ${PRICE.t1.toLocaleString()}</option>
                  <option value="t2">Tier 2 – Private Indulgence — ${PRICE.t2.toLocaleString()}</option>
                  <option value="t3">Tier 3 – VIP Sanctuary — ${PRICE.t3.toLocaleString()}</option>
                </select>
              </div>
              <div>
              ...
              <div className="grid md:grid-cols-2 gap-2">
                ...
                <a href={podiaCheckoutUrl('podia_deposit_button',selectedTier)} target="_blank" rel="noreferrer" className="w-full text-center rounded-2xl border border-[#F1C376] text-[#F1C376] px-4 py-3 font-semibold hover:bg-[#F1C376]/10">Proceed to Secure Deposit (Podia)</a>
              </div>
            </form>
          </div>
        </div>
